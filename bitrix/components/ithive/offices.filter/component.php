	...
	while ($arProp = $rsProp->GetNext())
	{
		...
		if(in_array($arProp["CODE"],$arParams["PROPERTY_CODE"]) && $arProp["PROPERTY_TYPE"] != "F")
		{
			...
			if ($arProp["PROPERTY_TYPE"]=="E")
			{
				...
				//Добавим сортировку для Регионов и Городов: сначала по SORT, потом по алфавиту
				$arSort = array("SORT"=>"ASC", "NAME"=>"ASC");
				
				//REGION
				//echo "<pre>";print_r($_REQUEST['arrFilter_pf']['REG']);echo "</pre>";

				if($arProp["CODE"]=="REG"){
					$arFilter1 = Array("IBLOCK_ID"=>14, "ACTIVE"=>"Y");
				}

				if($arProp["CODE"]=="REG_CITY"){
					if($_REQUEST['arrFilter_pf']['REG']){
						$arFilter1 = Array("IBLOCK_ID"=>15,  "ACTIVE"=>"Y",  "PROPERTY_REGION"=>$_REQUEST['arrFilter_pf']['REG']);
					}else{
						$arFilter1 = Array("IBLOCK_ID"=>15,  "ACTIVE"=>"Y");
					}
					
				}

				if($arProp["CODE"]=="REG" || $arProp["CODE"]=="REG_CITY"){
				$arSelect1 = Array("ID", "IBLOCK_ID", "NAME", "DATE_ACTIVE_FROM","PROPERTY_*");//IBLOCK_ID и ID обязательно должны быть указаны, см. описание arSelectFields выше
				
				$res = CIBlockElement::GetList($arSort, $arFilter1, false, false, $arSelect1);
				while($ob = $res->GetNextElement()){ 
				 $arFields = $ob->GetFields();  
				//print_r($arFields);
				 $arProps1 = $ob->GetProperties();

				 $arrEnum[$arFields["ID"]] = $arFields["NAME"];
...
if($arProp["CODE"]!='PRODUCT_TYPE'):
			if ($arProp["MULTIPLE"]=="Y")
				$res .= '<select onchange="this.form.submit()" multiple name="'.$name.'[]" size="'.$arParams["LIST_HEIGHT"].'">';
			else
				$res .= '<select onchange="this.form.submit()" name="'.$name.'">';
			$res .= '<option value="">'.GetMessage("CC_BCF_ALL").'</option>';
			foreach($arProp["VALUE_LIST"] as $key=>$val)
			{
				$res .= '<option';
