...
if($arParams["SEF_MODE"] == "Y")
{
	...
	if($componentPage == "section" && isset($arResult["VARIABLES"]["SECTION_CODE"]) && strlen($arResult["VARIABLES"]["SECTION_CODE"]) > 1 && (!isset($_REQUEST["filter"]) || $_REQUEST["filter"] != "Y")) {
		...
		if($section = $res->GetNext()) {			
			if($section["DEPTH_LEVEL"] == 1 || $section["DEPTH_LEVEL"] == 2 || $section["DEPTH_LEVEL"] == 3 || $section["DEPTH_LEVEL"] == 4) {
				$sects = CIBlockSection::GetList(array(), array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "SECTION_ID" => $section["ID"]), false);
				$sections_2 = 0;
				while($se = $sects->GetNext())
					$sections_2++;
				if($sections_2 > 0) {
					$componentPage = "sections";
				}
