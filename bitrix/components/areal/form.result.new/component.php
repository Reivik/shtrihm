			...
			)
			{
				...
				if (check_bitrix_sessid())
				{
					...
					if($RESULT_ID = CFormResult::Add($arParams["WEB_FORM_ID"], $arResult["arrVALUES"]))
					{
						...
						CFormResult::Mail($RESULT_ID,"72");
						LocalRedirect($arParams["LIST_URL"]."?succsess=Y#thank");
		...
		$arResult = array_merge(
			...
			array(
				...
				"FORM_HEADER" => sprintf( // form header (<form> tag and hidden inputs)
					"<form class=\"filter_form introduction\" name=\"%s\" action=\"%s\" method=\"%s\" enctype=\"multipart/form-data\">",
