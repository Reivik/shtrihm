...
//pr($APPLICATION->GetProperty("SHOW_BANNERS"));
//pr($APPLICATION->ShowProperty("SHOW_BANNERS"));
//if($APPLICATION->GetPageProperty("SHOW_BANNERS") != "Y")
if($APPLICATION->GetProperty("SHOW_BANNERS") != "Y")
	return;
...
$obj = CIBlockElement::GetList(
	array(),
	array("IBLOCK_ID" => IB_BANNERS),
	false,
	false,
	array("ID","PROPERTY_PAGE")
);
$i = 0;
while($arBan = $obj->GetNext())
{
	if(substr_count($_SERVER["REQUEST_URI"],$arBan["PROPERTY_PAGE_VALUE"]))
	{
		$arrFilter["ID"][] = $arBan["ID"];
		$i++;
		if($i == 5)
			break;
	}
	if($arBan["PROPERTY_PAGE_VALUE"] == "" && count($arEmptyPage) < 5)
	{
		$arEmptyPage[] = $arBan["ID"];
	}
}
if($i == 0)
{
	$arrFilter["ID"] = $arEmptyPage;
}
