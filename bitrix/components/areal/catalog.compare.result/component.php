...
if(isset($_REQUEST["action"]))
{
	switch($_REQUEST["action"])
	{
		...
		case "DELETE_FROM_COMPARE_RESULT":
			if(isset($_REQUEST["compare_id"]) && !empty($_REQUEST["compare_id"])) {
				$compare_str = implode("-", array_keys($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"]));
				$compare_id = explode("-", $_REQUEST["compare_id"]);
				foreach($arID as $ID) {
					if(in_array($ID, $compare_id))
						unset($compare_id[array_search($ID, $compare_id)]);
					unset($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"][$ID]);
				}
				if($compare_id)
					$compare_str = implode("-", $compare_id);
				LocalRedirect($APPLICATION->GetCurPage(false)."?compare_id=".$compare_str);				
			}
			else {			
				foreach($arID as $ID)
					unset($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"][$ID]);
...
//если сравниваемые идентификаторы хранятся не в сессии, а в гет-параметрах
if(isset($_REQUEST["compare_id"])) {
	$ID = explode("-", $_REQUEST["compare_id"]);
	foreach($ID as $key_compare) {
		//SELECT
		$arSelect = array(
			"ID",
			"IBLOCK_ID",
		...
		{
			$arMaster["NAME"] = $arElement["NAME"];
			$arCompare[$key_compare] = $arMaster;
		}
		elseif($arElement)
		{
			$arCompare[$key_compare] = $arElement;
		}
	}
}
else
