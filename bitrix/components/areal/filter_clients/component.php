...
if(CModule::IncludeModule("iblock"))
{
	...
	$cache_time = 3600;
	$arResult = array();
	$cache_dir_id = 'clients_filter';
	$cache_dir_path = '/clients_filter/';
	if ($cache_time > 0 && $cache->InitCache($cache_time, $cache_dir_id, $cache_dir_path))
	{
		$res = $cache->GetVars();
		if (is_array($res["clients_filter"]) && (count($res["clients_filter"]) > 0))
			$arResult = array_merge($arResult, $res["clients_filter"]);
	}
	else 
	{
		...
		if ($cache_time > 0) {
			...
			$cache->EndDataCache(array("clients_filter" => $arResult));
		}
	}
