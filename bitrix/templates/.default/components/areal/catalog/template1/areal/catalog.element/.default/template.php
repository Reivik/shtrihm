...
<div class="product">
	...
	<?if ($arResult["PROPERTIES"]["ONLINE_IN"]["VALUE"]) {?>
		...
		<div class="main_info right_info">
			<span class="pricer">*Цена без ФН</span><span> <?=$arResult["PROPERTIES"]["PRICE_IN"]["VALUE"]?> р</span>
		</div>
<?}?>
			<?if($arResult["NAME"]=="Онлайн касса Ростелеком"){?>
				<a onclick="OpenForm2<?if ($arResult["PROPERTIES"]["SPEC_LINK"]["VALUE"]) {?>New<?}?>();" class="right_but cat_button">Заказать</a>
			<?}else{?>
		...
		<?//echo"<pre>";print_r($arResult["PROPERTIES"]["PRICE_IN"]);echo"</pre>";?>
		function OpenForm() {
		...
		/*setTimeout(function(){
				$('input[name=form_text_218]').search_place_input({regions : region_db,});
				$('input[name=form_text_217]').search_place_input({cityes : city_db,});
			}, 100);*/
		$(".form_zakaz_new .title_form").html('Заявка на кассу Ростелеком')
		setTimeout(function(){
			$(".form_zakaz_new .sum_online_till").html('')
		}, 100);

		product_online_till_add_ros22('<?=$arResult["NAME"]?>');
		...
		<div>
			...
			if(!empty($arResult["PROPERTIES"])) {
				foreach($arResult["PROPERTIES"] as $key => $pr) {
					if($pr["VALUE"] && $pr["PROPERTY_TYPE"] != "F" && $key != 'MORE_PHOTO' && $key != 'other_product' && $key != 'dop_chars' && $key != 'type' && $key != 'keywords' && $key != 'description' && $key != 'seo_title' && $key != 'accreditation' && $key != 'PRICE_IN' && $key != 'SPEC_LINK' && $key != 'ONLINE_IN' && $key != 'FZ54_IN' && $key != 'DETAILS_URL_CUSTOM' && $key != 'pricebezfn' && $key != 'FILTER_ON' && $key != 'VIDEO_LINK' && $key != 'PRICE_DESC' && $key != 'NEW_POPUP') {
						$arResult["TTH"][] = $pr;
					}
				}
			}
