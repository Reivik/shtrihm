	...
	<div class="forum-reply-fields">
		...
		<div class="forum-reply-field forum-reply-field-settings">
			...
			<div class="forum-reply-field-setting">
				<label class="label_check <?=($arResult["POST_VALUES"]["USE_SMILES"]=="Y") ? "c_on" : "c_off";?>" for="USE_SMILES<?=$arParams["form_index"]?>">
					<input name="USE_SMILES" id="USE_SMILES<?=$arParams["form_index"]?>" value="Y" type="checkbox" <?=($arResult["POST_VALUES"]["USE_SMILES"]=="Y") ? "checked=\"checked\"" : "";?>>
...
?>
			...
			<div class="forum-reply-field-setting">
				<label class="label_check <?=($arResult["POST_VALUES"]["COPY_TO_OUTBOX"]=="Y") ? "c_on" : "c_off";?>" for="COPY_TO_OUTBOX<?=$arParams["form_index"]?>">
					<input name="COPY_TO_OUTBOX" id="COPY_TO_OUTBOX<?=$arParams["form_index"]?>" value="Y" type="checkbox" <?=($arResult["POST_VALUES"]["COPY_TO_OUTBOX"]=="Y") ? " checked disabled " : "";?>>
					<?=GetMessage("F_COPY_TO_OUTBOX")?>
				</label>
			</div>
			
			<div class="forum-reply-field-setting">
				<label class="label_check <?=($arResult["POST_VALUES"]["REQUEST_IS_READ"]=="Y") ? "c_on" : "c_off";?>" for="REQUEST_IS_READ<?=$arParams["form_index"]?>">
					<input name="REQUEST_IS_READ" id="REQUEST_IS_READ<?=$arParams["form_index"]?>" value="Y" type="checkbox" <?=($arResult["POST_VALUES"]["REQUEST_IS_READ"]=="Y") ? "checked=\"checked\"" : "";?>>
					<?=GetMessage("F_REQUEST_IS_READ")?>
				</label>
			</div>	
				<?
	endif;
