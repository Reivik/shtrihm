	...
	?><div class="forum-reply-field forum-reply-field-switcher"><?
		...
		?><span class="forum-reply-field forum-reply-field-switcher-vote"><?
			?><a href="javascript:void(0);" onclick="return ShowVote(this);" <?
				?>onfocus="ShowVote(this);" tabindex="<?=$iIndex?>"><?=GetMessage("F_ADD_VOTE")?></a>
		</span><? }
	?></div><?}
?>
</div>
		...
		<div class="forum-reply-field forum-reply-field-text">
			...
			$APPLICATION->IncludeComponent(
				...
				Array(
					...
					"BUTTONS" => forumTextParser::GetEditorButtons(array('forum' => $arResult['FORUM'])),
...
?>
			<div class="forum-reply-field-setting">
				<label class="label_check <?=($arResult["MESSAGE"]["USE_SMILES"]=="Y") ? "c_on" : "c_off";?>" for="USE_SMILES<?=$arParams["form_index"]?>">
					<input name="USE_SMILES" id="USE_SMILES<?=$arParams["form_index"]?>" value="Y" type="checkbox" <?=($arResult["MESSAGE"]["USE_SMILES"]=="Y") ? "checked=\"checked\"" : "";?>>
					<?=GetMessage("F_WANT_ALLOW_SMILES")?>
				</label>
			</div>
<?
};
...
?>
			<div class="forum-reply-field-setting">
				<label class="label_check <?=($arResult["MESSAGE"]["TOPIC_SUBSCRIBE"]=="Y") ? "c_on" : "c_off";?>" for="TOPIC_SUBSCRIBE<?=$arParams["form_index"]?>">
					<input name="TOPIC_SUBSCRIBE" id="TOPIC_SUBSCRIBE<?=$arParams["form_index"]?>" value="Y" type="checkbox" <?=($arResult["MESSAGE"]["TOPIC_SUBSCRIBE"]=="Y") ? " checked disabled " : "";?>>
					<?=GetMessage("F_WANT_ALLOW_SMILES")?>
				</label>
			</div>
			<div class="forum-reply-field-setting">
				<label class="label_check <?=($arResult["MESSAGE"]["FORUM_SUBSCRIBE"]=="Y") ? "c_on" : "c_off";?>" for="FORUM_SUBSCRIBE<?=$arParams["form_index"]?>">
					<input name="FORUM_SUBSCRIBE" id="FORUM_SUBSCRIBE<?=$arParams["form_index"]?>" value="Y" type="checkbox" <?=($arResult["MESSAGE"]["FORUM_SUBSCRIBE"]=="Y") ? "checked=\"checked\"" : "";?>>
					<?=GetMessage("F_WANT_SUBSCRIBE_FORUM")?>
				</label>
			</div>
<?
};
...
?>
		<div class="forum-reply-buttons">
			<input name="send_button" type="submit" value="<?=$arResult["SUBMIT"]?>" tabindex="<?=$tabIndex++;?>" <?
				?>onclick="this.form.MESSAGE_MODE.value = 'NORMAL';" class="inputBtn ap_btn"/>
			<input name="view_button" type="submit" value="<?=GetMessage("F_VIEW")?>" tabindex="<?=$tabIndex++;?>" <?
				?>onclick="this.form.MESSAGE_MODE.value = 'VIEW';" class="inputBtn ap_btn"/>
		</div>
