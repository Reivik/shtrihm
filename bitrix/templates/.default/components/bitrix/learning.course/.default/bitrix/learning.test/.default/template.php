...
<?if (sizeof($arResult["ACCESS_ERRORS"])):?>
	<?foreach ($arResult["ACCESS_ERRORS"] as $error):?>
		<p><font class="errortext"><?echo $error?></font></p>
	<?endforeach?>
<?else:?>
	...
	<form name="learn_test_answer" action="<?=$arResult["ACTION_PAGE"]?>" method="post">
		...
		<?if ($arResult["TEST"]["PASSAGE_TYPE"] > 0 && $arResult["NAV"]["PREV_QUESTION"]):?>
			<input type="submit" name="previous" class="inpBtn" onClick="javascript:window.location='<?=CUtil::JSEscape($arResult["QBAR"][$arResult["NAV"]["PREV_QUESTION"]]["URL"])?>'; return false;" value="<?=GetMessage("LEARNING_BTN_PREVIOUS")?>" />
		<?endif?>

		<input type="submit" name="next" class="inpBtn" value="<?=GetMessage("LEARNING_BTN_NEXT")?>"<?if ($arResult["TEST"]["PASSAGE_TYPE"] == 0):?> OnClick="return <?php if ($arResult["QUESTION"]["QUESTION_TYPE"] == "R"):?>checkSorting('<?=GetMessage("LEARNING_INVALID_SORT_CONFIRM")?>');<?php else:?>checkForEmpty('<?php if ($arResult["QUESTION"]["QUESTION_TYPE"] == "T"):?><?=GetMessage("LEARNING_EMPTY_RESPONSE_CONFIRM")?><?php else:?><?=GetMessage("LEARNING_NO_RESPONSE_CONFIRM")?><?php endif?>');<?php endif?>"<?endif?>>
		&nbsp;&nbsp;&nbsp;
		<?php
		{
			?>
			<input type="submit" name="finish" class="inpBtn" value="<?=GetMessage("LEARNING_BTN_FINISH")?>" onClick="return confirm('<?=GetMessage("LEARNING_BTN_CONFIRM_FINISH")?>')">
			<?php
		}
		?>
	...
	<?if (intval($arResult["TEST"]["CURRENT_INDICATION"]) > 0):?>
	...
	<?elseif (strlen($arResult["ERROR_MESSAGE"]) > 0):?>
		...
		<?=bitrix_sessid_post()?>
		<input type="hidden" class="inpBtn" name="back_page" value="<?=$arResult["SAFE_REDIRECT_PAGE"]?>" />
		<input type="submit" class="inpBtn" name="next" value="<?=GetMessage("LEARNING_BTN_CONTINUE")?>">
		</form>
	<?else:?>
		...
		<form name="learn_test_start" method="post" action="<?=$arResult["ACTION_PAGE"]?>">
		<?=bitrix_sessid_post()?>
		<input type="hidden" name="back_page" class="inpBtn" value="<?=$arResult["SAFE_REDIRECT_PAGE"]?>" />
		<input type="submit" name="next" class="inpBtn" value="<?=GetMessage("LEARNING_BTN_START")?>">
		</form>
	<?endif?>
<?endif?>
</div>