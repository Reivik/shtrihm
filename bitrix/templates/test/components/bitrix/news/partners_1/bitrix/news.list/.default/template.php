				...
				<tr>
					...
					<td class="info_block">						
						...
						<p class="addintional_info">
							...
							<span>Статус: <?foreach($arItem["STATUS"] as $status):?><?if(!empty($status["PREVIEW_PICTURE"]["src"])):?><img class="img_status" src="<?=$status["PREVIEW_PICTURE"]["src"]?>" width="<?=$status["PREVIEW_PICTURE"]["width"]?>" height="<?=$status["PREVIEW_PICTURE"]["height"]?>" title="<?=$status["NAME"]?>" alt="<?=$status["NAME"]?>" /><?else:?><?=$status["NAME"]?><?endif;?>; <?endforeach?></span><br />
							<?endif;?>
