...
<!-- End Google Tag Manager -->
	...
	foreach($Webinar_up as $key => $value):
		$file = CFile::ResizeImageGet ($value['test'][0]['VALUE'], array('width'=>138, 'height'=>138), BX_RESIZE_IMAGE_EXACT, true);
		$Webinar_up[$key]['test'][0] = $file['src'];
	endforeach;
	...
	<div id="panel"><?$APPLICATION->ShowPanel();?></div>	
		<div id ="page" style="visibility: hidden;">
				...
				<div class="heder_right">
					<div class="heder_right_up">
						<a href="<?=$Webinar_up[0]['test'][1]['VALUE']?>" target="_balnk"><img id="my_chenel" src="<?=$Webinar_up[0]['test'][0]?>" alt="">
						<span class="play"></span></a>
						<p class="tematika">
							<?$APPLICATION->IncludeComponent("bitrix:main.include","",Array(
								"AREA_FILE_SHOW" => "file",
								"PATH" => SITE_DIR."include/Nsh_kanal.php"
								)
							);?>
						</p>
					</div>
					...
					<div class="heder_right_down">
						...
						<p class="online"><?$APPLICATION->IncludeComponent("bitrix:main.include","",Array(
								"AREA_FILE_SHOW" => "file",
								"PATH" => SITE_DIR."include/Online_conf.php"
								)
							);?></p>
						<span id="<?if($vals[0]['attributes']['STAGE']!= 'START'):?>online_play_no<?else:?>online_play<?endif?>"  class="play"></span>
						<a id="zapis" href="#q"><?$APPLICATION->IncludeComponent("bitrix:main.include","",Array(
								"AREA_FILE_SHOW" => "file",
								"PATH" => SITE_DIR."include/zapic_na_web.php"
								)
							);?></a>
					</div>
				</div>
